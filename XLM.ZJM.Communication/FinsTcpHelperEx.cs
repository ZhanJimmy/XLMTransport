...
*└────────────────────────────┘
*/
namespace XLM.ZJM.Communication
{
    ///  功能描述：
    /// <para>作者：jianmei.zhan</para>
    /// <para>邮箱：jianmei.zhan@yankon.com</para>
    /// <para>时间：2021/11/19 8:59:10</para>
    /// </summary>
    public sealed class FinsTcpHelperEx
    {
        private string _ip;
        private int _port;
        /// <summary>
        /// 构造函数
        /// </summary>
        public FinsTcpHelperEx(string ip, int port)
        {
            _ip = ip;
            _port = port;
        }
        ...
        public void Run()
        {
            if (string.IsNullOrEmpty(_ip)) HandleDisconnect();
        ...
        private void TcpConnect(IAsyncResult result)
        {
            try
            {
                ...
                else
                {
                    ...
                    HandleConnected?.Invoke(client.Client);
                }
            } catch (Exception ex) { HandleError?.Invoke($"{_ip}:{_port}-->>{ex.Message}", ex); };
        }

        /// <summary>
        /// 错误
        /// </summary>
        public Action<string, Exception> HandleError { get; set; }

        /// <summary>
        /// 连接委托
        /// </summary>
        public Action<Socket> HandleConnected { get; set; }

        /// <summary>
        /// 断开连接
        /// </summary>
        public Action HandleDisconnect { get; set; }
    }
    internal static class FinsTcpExtension
    {
        #region Fins
